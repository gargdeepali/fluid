...
const BulbView = dia.ElementView.extend({
  ...
  getGlassAnimation() {
    ...
    console.log("Glass element found:", glassEl);  // Debug log

    if (!glassEl) {
      console.error("Glass element not found for animation.");
      return;
    }
    const keyframes = {
      stroke: ["#edbc26"],
  ...
  toggleLight() {
    ...
    const glassAnimation = this.getGlassAnimation();
    if (glassAnimation) {
      glassAnimation.playbackRate = state;
    }
  }
});
...
console.log("Bulb 1:", bulb1);

const bulb2 = Bulb.create(40).position(150, 105);
console.log("Bulb 2:", bulb2);
